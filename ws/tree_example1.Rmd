...
Specify relationships with `var.monotone`: 1 - positive/increasing, 0 - none/arbitrary, -1 - negative/decreasing.

Specify number of trees, 10k - 100k. More complex models may need more but start somewhere.

`interaction.depth` - how big is each little individual tree. Fitting lots of little trees in long sequence.

Number of splits. How complex do we expect the interactions to be? Here, could be up to 5 interactions. Wouldn't go beyond 5-7 here, can get wonky.
...
Sometimes cross-validation `cv = ` in the does not work for no discernible reason.
...
Predict the number of rings based on the first row of abalone.test.
