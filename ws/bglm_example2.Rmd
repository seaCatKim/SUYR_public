...
newdata1 <- newdata %>% 
  ungroup %>% # gather emmeans draws groups by variable so need to ungroup
  group_by(RATIO) %>% 
  median_hdci()

newdata %>% 
  ggplot() +
  geom_line(aes(y = Prob, x = RATIO, group = .draw),
            alpha = 0.05, show.legend = FALSE) +
  geom_point(data = polis, aes(y = PA, x = RATIO), color = 'blue') +
  geom_line(data = newdata1, aes(y = Prob, x = RATIO), color = 'yellow') +
  theme_classic()
```
